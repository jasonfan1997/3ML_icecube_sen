...
p.add_argument("--wrkdir", default="/data/condor_builds/users/jasonfan/3ml_sensitivity_v42/flux/result/", type=str,
...
if index != 2:
    sen = np.load("/data/condor_builds/users/jasonfan/3ml_sensitivity_v42/flux/sen_v42_"+str(index)+".npy")
    threesig = np.load("/data/condor_builds/users/jasonfan/3ml_sensitivity_v42/flux/threesig_v42_"+str(index)+".npy")
else:
    sen = np.load("/data/condor_builds/users/jasonfan/3ml_sensitivity_v42/flux/sen_v42.npy")
    threesig = np.load("/data/condor_builds/users/jasonfan/3ml_sensitivity_v42/flux/threesig_v42.npy")
declist = np.arange(-85,85,2.5)
decindex=np.argwhere(declist==dec)[0][0]
print("Injected neutrino sen :{} , 3 sigma discovery: {}".format(round(sen[decindex],3),round(threesig[decindex],3)))
...
icecube=IceCubeLike("temp",data,data_handler,llh_factory,source,livetime=livetime*3600*24,verbose=False)
...
icecube_79=IceCubeLike("temp_79",data,data_handler_79,llh_factory_79,source,livetime=livetime*3600*24,verbose=False)
...
icecube_59=IceCubeLike("temp_59",data,data_handler_59,llh_factory_59,source,livetime=livetime*3600*24,verbose=False)               
...
icecube_40=IceCubeLike("temp_40",data,data_handler_40,llh_factory_40,source,livetime=livetime*3600*24,verbose=False)   
...
fivesigflux = threesigflux
print("Flux sen :{} , 3 sigma discovery: {}, 3 sigma discovery: {}".format(senflux,threesigflux,fivesigflux))
result=np.array([dec,sen[decindex],threesig[decindex],threesig[decindex],senflux,threesigflux,fivesigflux])
np.save(wrkdir+"/"+str(dec)+".npy",result)
#np.save("/data/condor_builds/users/jasonfan/3ml_sensitivity_v42/flux/"+str(dec)+".npy",result)
